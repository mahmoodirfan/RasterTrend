...
- Requires QGIS Python bindings, GDAL, NumPy and SciPy in the QGIS Python environment.
...
| File | Description |
|------|-------------|
| `sens_slope.tif` | Sen's Slope – all-pairs trend magnitude per input time step |
| `p_value.tif` | Two-tailed p-value per pixel |
| `kendall_tau.tif` | Standard mode: Kendall's tau-a; seasonal mode: S / Var(S), not conventional tau |
...
2. Copy the `RasterTrend` folder to your QGIS plugins directory:
   - In QGIS, use **Settings → User Profiles → Open Active Profile Folder**, then open or create `python/plugins/`. This also handles custom profiles and OS-specific paths.
...
- Seasonal MK accepts `2 × period` layers, but the current engine skips seasons with fewer than 3 observations. Supply at least `3 × period` layers for complete cycles.
